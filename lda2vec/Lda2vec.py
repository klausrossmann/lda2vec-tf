...
class Lda2vec:
    ...
    def compute_normed_embeds(self):
        ...
        norm = tf.sqrt(tf.reduce_sum(self.mixture.topic_embedding ** 2, 1, keepdims=True))
        self.normed_embed_dict['topic'] = self.mixture.topic_embedding / norm
        norm = tf.sqrt(tf.reduce_sum(self.w_embed.embedding ** 2, 1, keepdims=True))
        self.normed_embed_dict['word'] = self.w_embed.embedding / norm
        norm = tf.sqrt(tf.reduce_sum(self.mixture.doc_embedding ** 2, 1, keepdims=True))
